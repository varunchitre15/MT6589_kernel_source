...
//<2013/03/12-22697-alberthsiao, Add BF3905 and IMX111 camera drivers
	...
	{
		PK_DBG("kdCISModulePowerOn get in---  IMX111_MIPI_RAW \n");
 ...
 //           if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
        ...
        PK_DBG("[ON_general 1.2V]sensorIdx:%d \n",SensorIdx);
...
//            mdelay(10);
	...
	{
		PK_DBG("kdCISModulePowerOn get in---  BF3905_MIPI_YUV \n");
		if (pinSetIdx==0)
	    	return 0;
        //enable active sensor
        //RST pin
...
//            mdelay(10);
        ...
        if(pinSetIdx == 0 || pinSetIdx == 2) {//disable sub
	        ...
	        if (GPIO_CAMERA_INVALID != pinSet[0][IDX_PS_CMRST]) {
				PK_DBG("kdCISModulePowerOn BF3905 close index[0]\n");
	        ...
	        if (GPIO_CAMERA_INVALID != pinSet[2][IDX_PS_CMRST]) {
				PK_DBG("kdCISModulePowerOn BF3905 close index[2]\n");
	...
	{
		PK_DBG("kdCISModulePowerOn get in---  S5K4E1GA_MIPI_RAW \n");
        //enable active sensor
        //RST pin
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
            if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
...
//            mdelay(10);
    ...
    else
    {
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_IMX111_MIPI_RAW \n");
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_BF3905_MIPI_YUV \n");
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_S5K4E1GA_MIPI_YUV \n");
